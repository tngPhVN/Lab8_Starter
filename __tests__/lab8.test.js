...
describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 1, l = prodItems.length; i < l; i++){
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let root = await prodItems[0].getProperty('shadowRoot');
    let addButton = await root.$('button');
    // Once you have the button, you can click it and check the innerText property of the button.
    // Once you have the innerText property, use innerText.jsonValue() to get the text value of it
    await addButton.click();
    let textButton = await addButton.getProperty('innerText');
    let textValue = await textButton.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // Loop thorugh every <product-item> elements
    for (let i = 0, l = prodItems.length; i < l; i++){
      let root = await prodItems[i].getProperty('shadowRoot');
      let addButton = await root.$('button');
      
      // Check to make sure button say "Remove from Cart"
      let textButton = await addButton.getProperty('innerText');
      let textValue = await textButton.jsonValue();
      expect(textValue).toBe('Remove from Cart');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0, l = prodItems.length; i < l; i++){
      let root = await prodItems[i].getProperty('shadowRoot');
      let removeButton = await root.$('button');
